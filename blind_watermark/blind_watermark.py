...
class WaterMark:
    ...
    def read_wm(self, wm_content, mode='img'):
        ...
        if mode == 'img':
            ...
            # Đọc hình mờ ở định dạng hình ảnh và chuyển đổi nó sang định dạng bit một chiều, loại bỏ mức xám
            self.wm_bit = wm.flatten() > 128

        elif mode == 'str':
            # wm_content.encode('utf-8').hex() - Chuyển đổi nội dung của chuỗi thành một chuỗi hex.
            # int(wm_content.encode('utf-8').hex(), base=16) - Chuyển đổi chuỗi hex thành một số nguyên.
            # bin(int(wm_content.encode('utf-8').hex(), base=16))[2:] - Chuyển đổi số nguyên thành một chuỗi nhị phân.
            byte = bin(int(wm_content.encode('utf-8').hex(), base=16))[2:]
            
            # Chuyển đổi chuỗi nhị phân thành một mảng Numpy, trong đó các giá trị là 'True' nếu bit tương ứng là 1 và 'False' nếu là 0.
    ...
    def extract(self, filename=None, embed_img=None, wm_shape=None, out_wm_name=None, mode='img'):
        ...
        #Chuyển đổi sang định dạng đã chỉ định:
        if mode == 'img':
